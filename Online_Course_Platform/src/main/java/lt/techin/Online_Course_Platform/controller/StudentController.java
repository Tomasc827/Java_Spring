...
public class StudentController {
  ...
  @PutMapping("/{id}")
  public ResponseEntity<Student> updateStudent(@Valid @RequestBody StudentDTO dto, @PathVariable long id) {
    Student updateStudent = studentService.updateStudent(dto, id);
    URI location = WebUtils.createLocation("/{id}", updateStudent.getId());
    return ResponseEntity.created(location).body(updateStudent);
